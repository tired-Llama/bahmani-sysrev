...
df_test <- df %>% 
    ...
    # //STUB - acuuracy and AUC
    mutate(N = test_N_image) %>% 
    mutate(AUC = test_ROC.AUC) %>% 
    mutate(Accuracy = test_Accuracy) %>%
    select(-matches("^test_"))  #   remove columns with data on model test
...
df_train <- df %>% 
    ...
    # //STUB - calculate the missing data
    mutate(TP = ifelse(is.na(training_True_positives),round(training_N_image*training_sensetivity*training_prevalence_of_lesion_in_training_dataset),training_True_positives)) %>% 
    mutate(FN = ifelse(is.na(training_False_Negatives),round(training_N_image*(1-training_sensetivity)*training_prevalence_of_lesion_in_training_dataset),training_False_Negatives)) %>% 
    mutate(FP = ifelse(is.na(training_False_positives),round(training_N_image*(1-training_specificity)*training_prevalence_of_lesion_in_training_dataset),training_False_positives)) %>% 
    mutate(TN = ifelse(is.na(training_True_negatives),round(training_N_image*training_specificity*training_prevalence_of_lesion_in_training_dataset),training_True_negatives)) %>% 
    # //STUB - acuuracy and AUC
    mutate(N = training_N_image) %>% 
    mutate(AUC = training_ROC.AUC) %>% 
    mutate(Accuracy = training_Accuracy) %>%
    select(-matches("^training_"))  #   remove columns with data on model training
...
df_validation <- df %>% 
    ...
    # //STUB - calculate the missing data
    mutate(TP = ifelse(is.na(validation_True_positives),round(validation_N_image*validation_sensetivity*validation_prevalence_of_lesion_in_training_dataset),validation_True_positives)) %>% 
    mutate(FN = ifelse(is.na(validation_False_Negatives),round(validation_N_image*(1-validation_sensetivity)*validation_prevalence_of_lesion_in_training_dataset),validation_False_Negatives)) %>% 
    mutate(FP = ifelse(is.na(validation_False_positives),round(validation_N_image*(1-validation_specificity)*validation_prevalence_of_lesion_in_training_dataset),validation_False_positives)) %>% 
    mutate(TN = ifelse(is.na(validation_True_negatives),round(validation_N_image*validation_specificity*validation_prevalence_of_lesion_in_training_dataset),validation_True_negatives)) %>% 
    # //STUB - acuuracy and AUC
    mutate(N = validation_N_image) %>% 
    mutate(AUC = validation_ROC.AUC) %>% 
    mutate(Accuracy = validation_Accuracy) %>% 
    select(-matches("^validation_"))  #   remove columns with data on model validation

df_all <- df_train %>% rbind(df_validation) %>% rbind(df_test)
# View(df_test)
# colnames(df_train)
